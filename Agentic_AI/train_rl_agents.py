...
def main():
    ...
    if ope_cfg.get("enabled", False) or args.ope_methods:
        ...
        # Select complete episodes that fit within the budget
        max_transitions = min(10000, len(trans_data["observations"]))
        dones_arr = trans_data["dones"]
        ep_boundaries = np.where(dones_arr)[0]
        if len(ep_boundaries) > 0:
            # Find how many complete episodes fit within the budget
            cutoff = ep_boundaries[ep_boundaries < max_transitions]
            if len(cutoff) > 0:
                n_ope = int(cutoff[-1]) + 1  # include the final done=True step
            else:
                n_ope = min(max_transitions, len(dones_arr))
        else:
            n_ope = min(max_transitions, len(dones_arr))
        ope_data = {k: v[:n_ope] for k, v in trans_data.items()}
        logger.info("OPE data: %d transitions, %d episodes",
                     n_ope, int(ope_data["dones"].sum()))

        # Determine the eval agent for actual Q-value log-probs
        ope_eval_agent = agent if algo_name != "DT" else None
