...
def run_ope(
    ...
    with torch.no_grad():
        ...
        acts_t = torch.as_tensor(acts, dtype=torch.long)
        beh_lp = behavior_policy.net.get_log_probs(obs_t, acts_t).numpy()

    # eval policy log probs (approximate via predicted actions)
    act_dim = int(acts.max()) + 1
    eval_lp = np.zeros_like(beh_lp)
    # approximate: assume eval policy picks the action it would pick with prob ~0.9
    for i in range(len(obs)):
        pred = eval_policy_fn(obs[i])
        if pred == acts[i]:
            eval_lp[i] = np.log(0.9)
        else:
            eval_lp[i] = np.log(0.1 / max(1, act_dim - 1))  # spread over other actions
