    ...
    def convert(self, df: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        for hh in households:
            ...
            for i in range(len(hh_df) - 1):
                ...
                # Mark episode boundary at day transitions (midnight)
                cur_date = row["ts"].date() if hasattr(row["ts"], "date") else None
                nxt_date = nxt["ts"].date() if hasattr(nxt["ts"], "date") else None
                is_day_boundary = (cur_date is not None and nxt_date is not None
                                   and cur_date != nxt_date)
                all_done.append(is_day_boundary)
            # mark last step of each household as done
            if len(hh_df) > 0:
                all_done[-1] = True
