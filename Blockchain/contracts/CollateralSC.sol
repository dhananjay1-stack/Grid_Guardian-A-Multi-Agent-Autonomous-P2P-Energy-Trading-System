...
contract CollateralSC is EIP712, AccessControl, ReentrancyGuard {
    ...
    // ── Fund locking for settlement ─────────────────────────────────
    mapping(bytes32 => uint256) public tradeLocks;                        // tradeId => locked amount
    mapping(bytes32 => bytes32) public tradeLockNode;                     // tradeId => nodeId whose funds are locked
    ...
    event FundsLockedForTrade(bytes32 indexed tradeId, bytes32 indexed nodeId, uint256 amount);
    event FundsUnlockedForTrade(bytes32 indexed tradeId, bytes32 indexed nodeId, uint256 amount);
    event FundsTransferredForTrade(bytes32 indexed tradeId, address indexed to, uint256 amount);
    ...
    /**
     * @notice Lock a node's deposited funds for a specific trade.
     * @param nodeId  Node whose deposit to lock
    ...
    function lockFunds(
        ...
        if (amount == 0) revert ZeroAmount();
        if (deposits[nodeId] < amount) revert InsufficientDeposit();
        if (tradeLocks[tradeId] != 0) revert TradeAlreadyLocked();

        deposits[nodeId] -= amount;
        tradeLocks[tradeId] = amount;
        tradeLockNode[tradeId] = nodeId;

        emit FundsLockedForTrade(tradeId, nodeId, amount);
    }

    /**
     * @notice Unlock and return funds to node's deposit (refund path).
     * @param tradeId The trade to unlock
     */
    function unlockFunds(bytes32 tradeId) external onlyRole(SETTLEMENT_ROLE) nonReentrant {
        uint256 amount = tradeLocks[tradeId];
        if (amount == 0) revert NoLockedFunds();
        bytes32 nodeId = tradeLockNode[tradeId];

        tradeLocks[tradeId] = 0;
        tradeLockNode[tradeId] = bytes32(0);
        deposits[nodeId] += amount;

        emit FundsUnlockedForTrade(tradeId, nodeId, amount);
    }

    /**
     * @notice Transfer locked funds to seller address (settlement path).
     * @param tradeId The trade whose locked funds to transfer
    ...
    function transferLockedFunds(
        ...
        emit FundsTransferredForTrade(tradeId, to, amount);
    }
}
